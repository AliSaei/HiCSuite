...
join2maps <- function(mat, seq, subseq, binsize, name = "chr", direction = "Forward", output_dir = NULL, output = "data"){
...
JoinMultiMaps <- function(mat, seq, subseq = NULL, name = 1, direction = "Forward", output_dir = NULL, output = list("data", "graph"), binsize){
 ...
 if(length(d) > 0 ) stop(paste0('"', paste0(d, collapse = ","),'"', "does not exist"))
  ...
  if(n < 3) {
    combined_maps <- join2maps(mat, maps[1], maps[2], direction = direction ,output_dir = output_dir, binsize = binsize, output = output)
  } else {
    if(direction == "Backward") maps <- gsub("^\\s+|\\s$", "", c(subseq, seq))
    
    combined_maps <- join2maps(mat, maps[1], maps[2], binsize = binsize,  output = "data")
    for(i in 3:n) {
      ...
      sel_mat <-   mat[rname %in% seq_name & mrnm %in% seq_name,] %>% 
        ...
        .[, ':='(pos = ifelse(rname == seq_name[i],  pos + max_len,
                 ...
                 mpos = ifelse(mrnm == seq_name[i],  mpos +  max_len ,
                                     ifelse(mrnm %in% len$rname, mpos +  len$len[len$rname == mrnm], mpos))),  by = .(rname, pos, mrnm, mpos)] %>%
        .[,':='(chr = paste0("chr", name))]
